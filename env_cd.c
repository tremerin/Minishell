...
/* var_name= PWD o OLDPWD antes o despues de cambiar la ruta */
void	set_env_pwd(t_mini *mini, char *var_name)
{
	char	cwd[1000];
	char	*pwd;
...
int cd(t_mini *mini, char *new_path)
{
	char	str[100];
	char	*pwd;
	int	returned;
	...
	pwd = expand_var("OLDPWD", mini->env);
	printf("%s\n", pwd);
	returned = chdir(new_path);
	if (returned != 0)
	{
		perror("error: "); 
	}
	set_env_pwd(mini, "PWD=");
	pwd = expand_var("PWD", mini->env);
	printf("%s\n", pwd);
	return (0);
}
